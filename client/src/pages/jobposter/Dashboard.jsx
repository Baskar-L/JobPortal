// src/pages/jobposter/Dashboard.jsx
import React from 'react';
import { motion } from 'framer-motion';
import { useNavigate } from 'react-router-dom';
import { Briefcase, FileText, Users, LogOut } from 'lucide-react';

const dashboardItems = [
  { icon: <Briefcase className="w-6 h-6" />, title: 'Posted Jobs', path: '/jobposter/jobs' },
  { icon: <FileText className="w-6 h-6" />, title: 'Applications', path: '/jobposter/applications' },
  { icon: <Users className="w-6 h-6" />, title: 'Manage Team', path: '/jobposter/team' },
];
        ...
        {dashboardItems.map((item, index) => (
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">View and manage {item.title.toLowerCase()}</p>
            </div>
          </motion.div>
        ))}
      </div>
